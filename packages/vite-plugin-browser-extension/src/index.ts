	...
	return [
		{
			...
			load(id) {
				...
				const contentScripts = parseResult.data.content_scripts ?? [];
				for (const script of contentScripts) {
					...
					if (js) {
						...
						for (const jsOrTsScriptFileName of js) {
							...
							const jsScriptFileName = jsOrTsScriptFileName.replace(TS_REGEX, '.js')
					...
					if (css) {
						for (const cssScriptFileName of css) {
							const resolvedId = path.resolve(dirname, cssScriptFileName);
				...
				return [
					...
					JSON.stringify(JSON.stringify(parseResult.data, null, 4)),
					', "---MANIFEST_END---")',
				].join('\n');
			},
			generateBundle(__outputOptions, bundle) {
				...
				manifestChunk.code = JSON.stringify(manifestResult.value, null, 4);
			},
		},
	];
}
