 ...
 */
export function manifestBuild(): Plugin {
	let logger: Logger;
	let root: string;
	return {
		apply: 'build',
		name: 'vite-plugin-browser-extension:manifest-build',
		config() {
			return {
				build: {
					rollupOptions: {
		...
		load(id) {
			...
			const parseResult = manifestSchema.safeParse(parseJsonResult.value);
			...
			for (const script of contentScripts) {
				...
				if (js) {
					...
					for (const jsOrTsScriptFileName of js) {
						...
						const jsScriptFileName = jsOrTsScriptFileName.replace(
							TS_REGEX,
							'.js',
						);
